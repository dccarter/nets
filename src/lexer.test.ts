...
function expectTokens(lexer: Lexer, ...all: { id: Tok, value?: number | boolean | string }[]) {
...
describe("Lexer", () => {
    describe("String Expression", () => {
        ...
        test("No expressions", () => {
            ...
            expectTokens(lexer, { id: Tok.LStrExpr }, { id: Tok.StrLit, value: "Hello World" }, { id: Tok.RStrExpr })
        })

        test("Expressions Only", () => {
            const lexer = createLexer("`${a}${10}${'b'}`")
            expectTokens(lexer, { id: Tok.LStrExpr },
                { id: Tok.Ident, value: "a" },
                { id: Tok.IntLit, value: 10 },
                { id: Tok.CharLit, value: Ascii['b'] },
                { id: Tok.RStrExpr })
        })

        test("Expression And Strings", () => {
            const lexer = createLexer("`Your name is ${name}, you are ${age} years old`")
            expectTokens(lexer, { id: Tok.LStrExpr },
